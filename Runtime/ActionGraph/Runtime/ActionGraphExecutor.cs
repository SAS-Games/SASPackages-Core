...
public sealed class ActionGraphExecutor : IActionGraphExecutionController, IDisposable
{
    ...
    private Awaitable _currentExecution;

    public bool HasGraph => _graph != null;
    public bool IsExecuting => _currentExecution != null;
    ...
    public async Awaitable ExecuteAsync(ActionContext context)
    {
        if (_graph == null)
        ...
        try
        {
            Awaitable execution = _graph.ExecuteAsync(context, cts.Token);
